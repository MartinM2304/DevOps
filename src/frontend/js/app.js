...
async function loadProjects() {
  ...
  try {
        ...
        if (form) {
          form.addEventListener("submit", async function (e) {
            ...
            try {
              ...
              if (result.status === "ok") {
                showToast(
                  "Изискването е добавено успешно към всички проекти",
                  "success"
                );
    ...
    console.error("Грешка при зареждане на проектите:", error);
    select.innerHTML = '<option value="">Грешка при зареждане</option>';
  } finally {
...
async function loadProjectData(projectId) {
  try {
    ...
    if (requirementsResponse.status !== "ok") {
      throw new Error(
        `Грешка при зареждане на изискванията: ${requirementsResponse.message}`
      );
    }
    currentRequirements = requirementsResponse.requirements || [];
    requirementStatistics = Array.isArray(currentRequirements)
      ? [...currentRequirements]
      : [];

    try {
      console.log("getDiagramsByProject load diagrams");
      const diagramsResponse = await window.apiManager.getDiagramsByProject(
        projectId
      );
      if (!diagramsResponse || diagramsResponse.status !== "ok") {
        throw new Error(
          `Грешка при зареждане на диаграмите: ${diagramsResponse.message}`
        );
      } else if (
        diagramsResponse.data &&
        Array.isArray(diagramsResponse.data)
      ) {
...
async function loadProjectMembers(projectId) {
  try {
    ...
    console.error(
      "Грешка при зареждане на данни за членове на проекта:",
      error
    );
...
function updateDashboard() {
  ...
  const stats = {
    ...
    functional: requirementStatistics.filter((req) => req.type === "functional")
      .length,
    nonFunctional: requirementStatistics.filter(
      (req) => req.type === "non-functional"
    ).length,
    highPriority: requirementStatistics.filter((req) => req.priority === "high")
      .length,
    mediumPriority: requirementStatistics.filter(
      (req) => req.priority === "medium"
    ).length,
    lowPriority: requirementStatistics.filter((req) => req.priority === "low")
      .length,
    byComplexity: {},
  };
  ...
  for (let i = 1; i <= 5; i++) {
    stats.byComplexity[i] = requirementStatistics.filter(
      (req) => req.complexity === i
    ).length;
  }
...
function setupEventListeners() {
  ...
  const deleteButtons = document.querySelectorAll(
    ".requirement-actions .btn-danger"
  );
  ...
  const indicatorsButtons = document.querySelectorAll(
    ".indicators-section button"
  );
...
async function handleRequirementSubmit(event) {
  ...
  try {
    ...
    const response = await window.apiManager.createRequirement(
      formData,
      currentProjectId
    );
    ...
    if (indicators.length > 0) {
      for (const indicator of indicators) {
        try {
          await window.apiManager.createIndicator({
            ...indicator,
            requirement_id: savedRequirement,
          });
        } catch (error) {
...
async function submitAddMember(event) {
  ...
  try {
    ...
    document.getElementById("add-member-error").textContent =
      err.message || "Грешка при добавяне на член.";
...
async function handleRegister(event) {
  ...
  try {
    showLoading(true);
    const result = await window.apiManager.registerUser({
      username,
      email,
      password,
    });
...
function collectIndicators() {
  ...
  const indicatorItems =
    indicatorsContainer.querySelectorAll(".indicator-item");
  ...
  indicatorItems.forEach((item) => {
    ...
    const descriptionInput = item.querySelector(
      'textarea[name="indicator_description[]"]'
    );
...
function displayLoggedUserDetails() {
  ...
  const projectSelection = document.querySelector(".project-selection");
  const mainInterface = document.getElementById("main-interface");
  const headerLoginRegisterBtns = document.getElementById(
    "login-register-btns"
  );
  const centeredLoginRegisterContainer = document.getElementById(
  ...
  if (userId && username) {
    ...
    if (centeredLoginRegisterContainer)
      centeredLoginRegisterContainer.style.display = "none";
    ...
    if (headerLoginRegisterBtns) headerLoginRegisterBtns.style.display = "none";
    if (centeredLoginRegisterContainer)
      centeredLoginRegisterContainer.style.display = "flex";
...
function hideLoggedUserDetails() {
  ...
  document.getElementById("centered-login-register-btns").style.display =
    "flex";
  document.getElementById("main-interface").style.display = "none";
}
...
window.onclick = function (event) {
  ...
  if (event.target === document.getElementById("register-modal"))
    closeRegisterModal();
  if (event.target === document.getElementById("indicators-modal"))
    closeIndicatorsModal();
        ...
        .map(
          (indicator) => `
                <div class="indicator-item">
                    ...
                    <p><strong>Единица:</strong> ${
                      indicator.unit || "Няма единица"
                    }</p>
                    <p><strong>Стойност:</strong> ${
                      indicator.value || "Няма стойност"
                    }</p>
                    <p><strong>Описание:</strong> ${
                      indicator.description || "Няма описание"
                    }</p>
                </div>
            `
        )
...
async function performTranslation() {
  ...
  try {
    const result = await api.translateRequirement(
      currentTranslationRequirementId,
      targetLang
    );
        ...
        <div class="requirement-card" style="margin: 10px 0;">
            <div class="requirement-header">
                ...
                <div class="requirement-meta">
                    <span class="priority">${getPriorityText(
                      translatedReq.priority
                    )}</span>
            ...
            <div class="requirement-body">
                ...
                ${
                  translatedReq.component
                    ? `<div class="component">Компонент: ${escapeHtml(
                        translatedReq.component
                      )}</div>`
                    : ""
                }
                ...
                ${
                  translatedReq.assignee
                    ? `<div class="assignee">Отговорен: ${escapeHtml(
                        translatedReq.assignee
                      )}</div>`
                    : ""
                }
                ...
                ${
                  translatedReq.complexity
                    ? `<div class="complexity">Сложност: ${getComplexityText(
                        translatedReq.complexity
                      )}</div>`
                    : ""
                }
