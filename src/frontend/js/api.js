...
class API {
  ...
  async translateRequirement(requirementId, targetLang = "en") {
    ...
    try {
      ...
      throw new Error(raw || "Невалиден отговор от услугата за превод");
    }

    if (!response.ok || (data && data.status === "error")) {
      throw new Error((data && data.message) || "Грешка при превод");
    }
  ...
  compress(s) {
    ...
    document.getElementById("diagram-image").src =
      "http://www.plantuml.com/plantuml/img/" + this.encode64(deflate(s, 9));
  }
...
@enduml`;
  ...
  async generatePlantUMLDiagram(umlText, format = "png") {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `PlantUML API error: ${response.status} ${response.statusText}`
        );
      }
  ...
  async addMemberToProject(projectId, username) {
    try {
      return this.post("project_members.php", {
